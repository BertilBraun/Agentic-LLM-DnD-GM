...
async def ensure_indexes() -> None:
    ...
    async with driver.session() as session:
        ...
        await result.consume()
        result = await session.run(
            'CREATE INDEX location_campaign IF NOT EXISTS FOR (n:Location) ON (n.campaign_id, n.name)'
        )
        await result.consume()
        result = await session.run(
            'CREATE INDEX faction_campaign IF NOT EXISTS FOR (n:Faction) ON (n.campaign_id, n.name)'
        )
        await result.consume()
