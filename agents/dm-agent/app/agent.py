...
async def run(campaign_id: str, player_message: str) -> str:
    ...
    # 4. Log DM turn
    log_resp = await call_mcp(STATE_MCP_URL, "log_turn", {"role": "dm", "content": dm.gm_speech}, campaign_id, LogTurnOut)
    dm_turn_id: str = log_resp.turn_id
    ...
    async def _background() -> None:
        async def _speak() -> None:
            try:
                r = await call_mcp(MEDIA_MCP_URL, "speak",
                                   {"text": dm.gm_speech, "voice_id": DM_VOICE_ID,
                                    "voice_instructions": DM_VOICE_INSTRUCTIONS},
                                   campaign_id, SpeakOut, timeout=90)
                if r.file_path:
        ...
        async def _image() -> None:
            try:
                r = await call_mcp(MEDIA_MCP_URL, "generate_image",
                                   ...
                                   campaign_id, ImageOut, timeout=120)
                if r.file_path:
        ...
        async def _memory_and_world() -> None:
            ...
            try:
                await call_mcp(KNOWLEDGE_MCP_URL, "update_world", {"narrative_text": dm.gm_speech}, campaign_id, UpdateWorldOut)
            except Exception:
...
async def _handle_npc(campaign_id: str, invoke_npc: InvokeNpc, visual_style: str) -> None:
    save_resp = await call_mcp(STATE_MCP_URL, "save_npc", {"npc_json": invoke_npc.model_dump()}, campaign_id, SaveNpcOut)
    ...
    if portrait_path:
        await call_mcp(STATE_MCP_URL, "save_npc",
                       {"npc_json": invoke_npc.model_dump(), "portrait_path": portrait_path}, campaign_id, SaveNpcOut)
