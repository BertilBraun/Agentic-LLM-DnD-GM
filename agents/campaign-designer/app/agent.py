...
async def run(campaign_id: str, player_message: str) -> str:
    ...
    if player_message:
        ...
        await call_mcp(
            STATE_MCP_URL, 'log_turn', {'role': 'player', 'content': player_message}, campaign_id, LogTurnOut
        )
    ...
    if done:
        ...
        try:
            ...
            await call_mcp(
                KNOWLEDGE_MCP_URL, 'update_world', {'narrative_text': seed_text}, campaign_id, UpdateWorldOut
            )
        except Exception:
