...
async def run(campaign_id: str, player_message: str) -> str:
    ...
    if conv_start_turn_id:
        pre_resp = await call_mcp(STATE_MCP_URL, "get_turns",
                                  {"before_turn_id": conv_start_turn_id, "limit": 3}, campaign_id, GetTurnsOut)
    ...
    if conv_start_turn_id:
        conv_resp = await call_mcp(STATE_MCP_URL, "get_turns",
                                   {"since_turn_id": conv_start_turn_id}, campaign_id, GetTurnsOut)
    ...
    if npc_turn.done:
        ...
        conv_transcript = "\n".join(
            _format_turn(t["role"] if isinstance(t, dict) else t.role,
        ...
        try:
            await call_mcp(KNOWLEDGE_MCP_URL, "update_world", {"narrative_text": conv_transcript}, campaign_id, UpdateWorldOut)
        except Exception:
